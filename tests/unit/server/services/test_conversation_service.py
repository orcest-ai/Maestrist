...
async def test_setup_without_tokens_non_saas_uses_user_secrets(mock_settings):
    """Test that Maestrist (OSS mode) uses user_secrets.provider_tokens when no tokens provided.

    This test verifies Maestrist (OSS mode) backward compatibility - tokens come from local config, not endpoint.
    """
    user_id = 'test_user_456'
    conversation_id = 'test_conv_123'
    providers_set = [ProviderType.GITHUB]
