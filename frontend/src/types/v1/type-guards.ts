...
import type { MaestristParsedEvent } from "../core/index";
 ...
 */
export const isMessageEvent = (event: MaestristEvent): event is MessageEvent =>
 ...
 */
export const isActionEvent = (event: MaestristEvent): event is ActionEvent =>
  event.source === "agent" &&
  "action" in event &&
  event.action !== null &&
...
/**
 * TEMPORARY: Type guard to check if an event is a V1 MaestristEvent
...
export function isV1Event(
  event: MaestristEvent | MaestristParsedEvent,
): event is MaestristEvent {
...
/**
 * TEMPORARY: Type guard to check if an event is a V0 MaestristParsedEvent
 *
 * @deprecated This is temporary until full V1 migration is complete
 */
