...
describe("useAppTitle", () => {
  ...
  it("should return 'Maestrist' if is OSS and NOT in /conversations", async () => {
    // @ts-expect-error - only returning partial config for test
    ...
    await waitFor(() => expect(result.current).toBe("Maestrist"));
  });

  it("should return 'Maestrist Cloud' if is SaaS and NOT in /conversations", async () => {
    // @ts-expect-error - only returning partial config for test
    getConfigSpy.mockResolvedValue({
      app_mode: "saas",
    });
    ...
    await waitFor(() => expect(result.current).toBe("Maestrist Cloud"));
  });

  it("should return '{some title} | Maestrist' if is OSS and in /conversations", async () => {
    // @ts-expect-error - only returning partial config for test
    ...
    await waitFor(() =>
      expect(result.current).toBe("My Conversation | Maestrist"),
    );
  });

  it("should return '{some title} | Maestrist Cloud' if is SaaS and in /conversations", async () => {
    // @ts-expect-error - only returning partial config for test
    ...
    await waitFor(() =>
      expect(result.current).toBe(
        "Another Conversation Title | Maestrist Cloud",
      ),
    );
  });
