This repository contains the code for Maestrist, an automated AI software engineer. It has a Python backend
...
## Running Maestrist with Maestrist:
...
The `enterprise/` directory contains additional functionality that extends the open-source Maestrist codebase. This includes:
...
**Setup Steps:**
1. First, build the main Maestrist project: `make build`
...
- The enterprise server extends the Maestrist server through dynamic imports
- Database changes require careful migration planning in `enterprise/migrations/`
- Always test changes in both Maestrist and enterprise contexts
...
- This ensures code works in both Maestrist and enterprise contexts
...
- For frontend issues, ensure the main Maestrist frontend is built: `make build`
...
Microagents are specialized prompts that enhance Maestrist with domain-specific knowledge and task-specific workflows. They are Markdown files that can include frontmatter for configuration.
...
- To add a new user setting to Maestrist, follow these steps:
...
There are two main patterns for saving settings in the Maestrist frontend:
...
To add a new LLM model to Maestrist, you need to update multiple files across both frontend and backend:
...
1. **Frontend Model Arrays** (`frontend/src/utils/verified-models.ts`):
   ...
   - Add to provider-specific arrays based on the model's provider:
     ...
     - `VERIFIED_OPENHANDS_MODELS` for models available through Maestrist provider
...
3. **Backend Model List** (`openhands/utils/llm.py`):
   - **CRITICAL**: Add the model to the `openhands_models` list (lines 57-66) if using Maestrist provider
...
- **VERIFIED_OPENHANDS_MODELS**: Models available through Maestrist managed provider
