...
if [ "$(id -u)" -ne 0 ]; then
  echo "The Maestrist entrypoint.sh must run as root"
  exit 1
fi
...
if [ -z "$WORKSPACE_MOUNT_PATH" ]; then
  # This is set to /opt/workspace in the Dockerfile. But if the user isn't mounting, we want to unset it so that Maestrist doesn't mount at all
...
if [[ "$SANDBOX_USER_ID" -eq 0 ]]; then
  echo "Running Maestrist as root"
